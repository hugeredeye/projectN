...
def explain_point(point: str) -> str:
    try:
        # Обновленный промпт для краткого пояснения
        explanation_prompt = """Вы - эксперт по техническим требованиям. 
        Перескажи пункт простыми словами в одном коротком предложении. 
        Не добавляй рассуждений, комментариев или диалогов, только суть.

        Примеры:
        Вход: "Программа должна выводить изображение с зелёной линией или сообщением об ошибке при некорректных входных данных."
        Выход: "Программа показывает путь зелёной линией или ошибку."

        Вход: "Программа должна обрабатывать данные за минуту."
        Выход: "Программа выполняет задачу за минуту."

        Вход: "Система должна сохранять данные в базе."
        Выход: "Система записывает данные в базу."

        Теперь перескажи этот пункт: {point}
        """
